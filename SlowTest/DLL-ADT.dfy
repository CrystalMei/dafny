...
datatype DList<A> = DList(
  ...
  ghost f:seq<AbInt>, // map index to pointer: f[i] = p (where p > 0)
  ghost g:seq<AbInt>  // map pointer to index: g[p] = i (where g[p] == -2 means p is unused and g[0] == -1 (sentinel))
  )
...
predicate Invs<A>(nodes:seq<Node<A>>, freeStack:uint64, s:seq<A>, f:seq<AbInt>, g:seq<AbInt>)
{
  && |f| == |s|
  && |g| == |nodes|
  && |nodes| > 0
  && g[0] == int2adt(sentinel)
  && 0 <= freeStack as int < |nodes|
  && (forall i :: 0 <= i < |f| ==> AbLt(int2adt(0), f[i]) && AbLt(f[i], int2adt(|nodes|)))
  && (forall i {:trigger g[f[i]]} :: 0 <= i < |f| ==> g[f[i]] == i) // TODO
  && (forall p :: 0 <= p < |g| ==>
    && (AbLt(int2adt(unused), g[p]) || int2adt(unused) == g[p]) && AbLt(g[p], int2adt(|s|))
    && 0 <= nodes[p].next as int < |g|
    && ((AbLt(int2adt(0), g[p]) || int2adt(0) == g[p]) <==> nodes[p].data.Some?))
  && (forall p :: 0 <= p < |g| && 
      (AbLt(int2adt(sentinel), g[p]) || int2adt(sentinel) == g[p]) ==>
    && (int2adt(sentinel) == g[p] ==> p == 0)
    && ((AbLt(int2adt(0), g[p]) || int2adt(0) == g[p]) ==> f[g[p]] == p && nodes[p].data == Some(s[g[p]])) // TODO
    && nodes[p].next as int == (
      if AbAdd(g[p], int2adt(1)) < int2adt(|f|) then f[g[p] + 1] // nonlast.next or sentinel.next // TODO
      else 0) // last.next == sentinel or sentinel.next == sentinel
    && nodes[p].prev as int == (
      if AbLt(int2adt(0), g[p]) then f[g[p] - 1] // nonfirst.prev // TODO
      else if int2adt(0) == g[p] || |f| == 0 then int2adt(0) // first.prev == sentinel or sentinel.prev == sentinel
...
function ValidPtr<A>(l:DList<A>, p:uint64):(b:bool)
  ensures b ==> p != 0
{
  0 < p as int < |l.g| && (AbLt(int2adt(0), l.g[p]) || l.g[p] == int2adt(0))
}
