  ...
  ensures var r := Split(xs, b); Length(xs) == Length(r.0) + Length(r.1)
{
  match xs
  case Nil =>
    assume Length<int>(Nil) + Length<int>(Nil) == Length<int>(Nil);
    (Nil, Nil)
  case Cons(x, tail) =>
    assume tail < xs;
    var (L, R) := Split(tail, b);
    if x < b then
      assume Length(Cons(x, L)) + Length(R) == Length(xs);
      (Cons(x, L), R)
    else
      assume Length(L) + Length(Cons(x, R)) == Length(xs);
      (L, Cons(x, R))
}
...
function Elements(xs: List): set // verified
{
  ...
  case Nil => {}
  case Cons(x, tail) =>
    assume tail < xs; {x} + Elements(tail)
}
