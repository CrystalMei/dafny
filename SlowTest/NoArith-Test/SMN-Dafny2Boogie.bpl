// Dafny 2.3.0.10506
// Command Line Options: /compile:0 /trace SMN_Dafny.dfy /print:SMN-Dafny2Boogie.bpl /proverLog:SMN-Dafny2z3.smt2 /proverOpt:O:smt.arith.solver=0
...
axiom 8 <= $FunctionContextHeight
   ==> (forall a#0: int, b#0: int :: 
    ...
    _module.__default.Add#canCall(a#0, b#0) || 8 != $FunctionContextHeight ==> true);
...
procedure CheckWellformed$$_module.__default.Add__Commutative__p2(a#0: int, b#0: int);
  free requires 12 == $FunctionContextHeight;
  modifies $Heap, $Tick;



procedure Call$$_module.__default.Add__Commutative__p2(a#0: int, b#0: int);
  modifies $Heap, $Tick;
  // user-defined postconditions
  free ensures _module.__default.Add#canCall(a#0, b#0)
     && _module.__default.Add#canCall(b#0, a#0);
  ensures _module.__default.Add(a#0, b#0) == _module.__default.Add(b#0, a#0);
  // frame condition
  free ensures old($Heap) == $Heap;



procedure CheckWellformed$$_module.__default.Add__Associative__p3(a#0: int, b#0: int, c#0: int);
  free requires 13 == $FunctionContextHeight;
  modifies $Heap, $Tick;



procedure Call$$_module.__default.Add__Associative__p3(a#0: int, b#0: int, c#0: int);
  modifies $Heap, $Tick;
  // user-defined postconditions
  free ensures _module.__default.Add#canCall(a#0, b#0)
     && _module.__default.Add#canCall(_module.__default.Add(a#0, b#0), c#0)
     && 
    _module.__default.Add#canCall(b#0, c#0)
     && _module.__default.Add#canCall(a#0, _module.__default.Add(b#0, c#0));
  ensures _module.__default.Add(_module.__default.Add(a#0, b#0), c#0)
     == _module.__default.Add(a#0, _module.__default.Add(b#0, c#0));
  // frame condition
  free ensures old($Heap) == $Heap;



procedure CheckWellformed$$_module.__default.Add__Identity__p1(a#0: int);
  free requires 14 == $FunctionContextHeight;
  modifies $Heap, $Tick;



procedure Call$$_module.__default.Add__Identity__p1(a#0: int);
  modifies $Heap, $Tick;
  // user-defined postconditions
  free ensures _module.__default.Add#canCall(a#0, LitInt(0))
     && _module.__default.Add#canCall(LitInt(0), a#0)
     && (_module.__default.Add(a#0, LitInt(0)) == _module.__default.Add(LitInt(0), a#0)
       ==> _module.__default.Add#canCall(LitInt(0), a#0));
  ensures _module.__default.Add(a#0, LitInt(0)) == _module.__default.Add(LitInt(0), a#0);
  ensures _module.__default.Add(LitInt(0), a#0) == a#0;
  // frame condition
  free ensures old($Heap) == $Heap;
  ...
  var b$reqreads#0: bool;
    ...
    else
    {
        ...
        else if (xs#0 == #_module.List.Cons(_mcc#0#0, _mcc#1#0))
        {
            ...
            // ----- assume statement ----- SMN_Dafny.dfy(24,5)
            assume true;
            assume DtRank(tail#4) < DtRank(xs#0);
            ##xs#0 := tail#4;
...
axiom 3 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, xs#0: DatatypeType, b#0: int :: 
    ...
    _module.__default.Split#canCall(xs#0, b#0)
         || (3 != $FunctionContextHeight && $Is(xs#0, Tclass._module.List(TInt)))
       ==> (var r#0 := _module.__default.Split($ly, xs#0, b#0); 
          _module.__default.Length(TInt, $LS($LZ), xs#0)
             == _module.__default.Length(TInt, $LS($LZ), $Unbox(_System.Tuple2._0(r#0)): DatatypeType)
...
procedure CheckWellformed$$_module.__default.Split(xs#0: DatatypeType where $Is(xs#0, Tclass._module.List(TInt)), b#0: int);
  ...
  ensures (var r#1 := _module.__default.Split($LS($LS($LZ)), xs#0, b#0); 
    _module.__default.Length(TInt, $LS($LS($LZ)), xs#0)
       == _module.__default.Length(TInt, $LS($LS($LZ)), $Unbox(_System.Tuple2._0(r#1)): DatatypeType)
...
implementation CheckWellformed$$_module.__default.Split(xs#0: DatatypeType, b#0: int)
{
  ...
  var b$reqreads#0: bool;
    ...
    assume {:captureState "SMN_Dafny.dfy(28,16): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    if (*)
    {
        ...
        assume (var r#1 := _module.__default.Split($LS($LZ), xs#0, b#0); 
          _module.__default.Length(TInt, $LS($LZ), xs#0)
             == _module.__default.Length(TInt, $LS($LZ), $Unbox(_System.Tuple2._0(r#1)): DatatypeType)
    ...
    else
    {
        ...
        if (xs#0 == #_module.List.Nil())
        {
            // ----- assume statement ----- SMN_Dafny.dfy(33,5)
            ##xs#11 := Lit(#_module.List.Nil());
            // assume allocatedness for argument to function
            assume $IsAlloc(##xs#11, Tclass._module.List(TInt), $Heap);
            assume _module.__default.Length#canCall(TInt, Lit(#_module.List.Nil()));
            ##xs#12 := Lit(#_module.List.Nil());
            // assume allocatedness for argument to function
            assume $IsAlloc(##xs#12, Tclass._module.List(TInt), $Heap);
            assume _module.__default.Length#canCall(TInt, Lit(#_module.List.Nil()));
            ##xs#13 := Lit(#_module.List.Nil());
            // assume allocatedness for argument to function
            assume $IsAlloc(##xs#13, Tclass._module.List(TInt), $Heap);
            assume _module.__default.Length#canCall(TInt, Lit(#_module.List.Nil()));
            assume _module.__default.Length#canCall(TInt, Lit(#_module.List.Nil()))
               && _module.__default.Length#canCall(TInt, Lit(#_module.List.Nil()))
               && _module.__default.Length#canCall(TInt, Lit(#_module.List.Nil()));
            assume LitInt(_module.__default.Length(TInt, $LS($LZ), Lit(#_module.List.Nil()))
                   + _module.__default.Length(TInt, $LS($LZ), Lit(#_module.List.Nil())))
               == LitInt(_module.__default.Length(TInt, $LS($LZ), Lit(#_module.List.Nil())));
        ...
        else if (xs#0 == #_module.List.Cons($Box(_mcc#0#0), _mcc#1#0))
        {
            ...
            // ----- assume statement ----- SMN_Dafny.dfy(36,5)
            assume true;
            assume DtRank(tail#4) < DtRank(xs#0);
            havoc L#4;
            ...
            if (x#4 < b#0)
            {
                // ----- assume statement ----- SMN_Dafny.dfy(39,7)
                ##xs#5 := #_module.List.Cons($Box(x#4), L#4);
                // assume allocatedness for argument to function
                assume $IsAlloc(##xs#5, Tclass._module.List(TInt), $Heap);
                assume _module.__default.Length#canCall(TInt, #_module.List.Cons($Box(x#4), L#4));
                ##xs#6 := R#4;
                // assume allocatedness for argument to function
                assume $IsAlloc(##xs#6, Tclass._module.List(TInt), $Heap);
                assume _module.__default.Length#canCall(TInt, R#4);
                ##xs#7 := xs#0;
                // assume allocatedness for argument to function
                assume $IsAlloc(##xs#7, Tclass._module.List(TInt), $Heap);
                assume _module.__default.Length#canCall(TInt, xs#0);
                assume _module.__default.Length#canCall(TInt, #_module.List.Cons($Box(x#4), L#4))
                   && _module.__default.Length#canCall(TInt, R#4)
                   && _module.__default.Length#canCall(TInt, xs#0);
                assume _module.__default.Length(TInt, $LS($LZ), #_module.List.Cons($Box(x#4), L#4))
                     + _module.__default.Length(TInt, $LS($LZ), R#4)
                   == _module.__default.Length(TInt, $LS($LZ), xs#0);
                assume _module.__default.Split($LS($LZ), xs#0, b#0)
            ...
            {
                // ----- assume statement ----- SMN_Dafny.dfy(42,7)
                ##xs#8 := L#4;
                // assume allocatedness for argument to function
                assume $IsAlloc(##xs#8, Tclass._module.List(TInt), $Heap);
                assume _module.__default.Length#canCall(TInt, L#4);
                ##xs#9 := #_module.List.Cons($Box(x#4), R#4);
                // assume allocatedness for argument to function
                assume $IsAlloc(##xs#9, Tclass._module.List(TInt), $Heap);
                assume _module.__default.Length#canCall(TInt, #_module.List.Cons($Box(x#4), R#4));
                ##xs#10 := xs#0;
                // assume allocatedness for argument to function
                assume $IsAlloc(##xs#10, Tclass._module.List(TInt), $Heap);
                assume _module.__default.Length#canCall(TInt, xs#0);
                assume _module.__default.Length#canCall(TInt, L#4)
                   && _module.__default.Length#canCall(TInt, #_module.List.Cons($Box(x#4), R#4))
                   && _module.__default.Length#canCall(TInt, xs#0);
                assume _module.__default.Length(TInt, $LS($LZ), L#4)
                     + _module.__default.Length(TInt, $LS($LZ), #_module.List.Cons($Box(x#4), R#4))
                   == _module.__default.Length(TInt, $LS($LZ), xs#0);
                assume _module.__default.Split($LS($LZ), xs#0, b#0)
...
implementation {:_induction xs#0, b#0} Impl$$_module.__default.Split__Correct(xs#0: DatatypeType, b#0: int) returns ($_reverifyPost: bool)
{
  ...
  var b##0_0: int;
    ...
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    assume {:captureState "SMN_Dafny.dfy(52,0): initial state"} true;
    ...
    else if (xs#0 == #_module.List.Cons($Box(_mcc#0#0_0), _mcc#1#0_0))
    {
        ...
        // ----- call statement ----- SMN_Dafny.dfy(56,18)
        ...
        assume {:captureState "SMN_Dafny.dfy(56,26)"} true;
    }
    else
    {
        assume false;
...
implementation CheckWellformed$$_module.__default.Elements(_module._default.Elements$_T0: Ty, xs#0: DatatypeType)
{
  ...
  var b$reqreads#0: bool;
    ...
    assume {:captureState "SMN_Dafny.dfy(59,9): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    if (*)
    {
    ...
    else
    {
        ...
        else if (xs#0 == #_module.List.Cons(_mcc#0#0, _mcc#1#0))
        {
            ...
            // ----- assume statement ----- SMN_Dafny.dfy(64,5)
            assume true;
            assume DtRank(tail#4) < DtRank(xs#0);
            ##xs#0 := tail#4;
            // assume allocatedness for argument to function
   ...
   returns ($_reverifyPost: bool)
{
  ...
  var ##xs#0_1: DatatypeType;
    ...
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    assume {:captureState "SMN_Dafny.dfy(70,0): initial state"} true;
    assume $IsA#_module.List(xs#0);
    $initHeapForallStmt#0 := $Heap;
    havoc $Heap, $Tick;
    assume $initHeapForallStmt#0 == $Heap;
    ...
    else if (xs#0 == #_module.List.Cons(_mcc#0#0_0, _mcc#1#0_0))
    {
        ...
        // ----- assume statement ----- SMN_Dafny.dfy(75,3)
        ##xs#0_0 := xs#0;
        // assume allocatedness for argument to function
...
implementation CheckWellformed$$_module.__default.NoDuplicates(_module._default.NoDuplicates$_T0: Ty, xs#0: DatatypeType)
{
  ...
  var b$reqreads#1: bool;
    ...
    assume {:captureState "SMN_Dafny.dfy(79,10): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    if (*)
    {
